...
export WAF_RG="preda-infra-vnet"
export WAF_POL_NAME="pol-appgw-ocp${CLUSTER_INSTANCE}-${TF_VAR_op_env}-${TF_VAR_location_code}-001"
export WAF_NAME=$(az network application-gateway list -g ${WAF_RG} --query [].name -o tsv)
...
az network application-gateway update  --resource-group $WAF_RG --name $WAF_NAME --set firewallPolicy.id=$WAF_POL_ID


# REQUEST-942-APPLICATION-ATTACK-SQLI search exceptions
# Remove first otherwise an error is thrown for duplicate rules
az network application-gateway waf-policy managed-rule exclusion rule-set remove --resource-group $WAF_RG --policy-name $WAF_POL_NAME \
...
az network application-gateway waf-policy managed-rule exclusion rule-set add --resource-group $WAF_RG --policy-name $WAF_POL_NAME \
  --match-operator Equals \
  --match-variable RequestArgNames \
  --selector search \
  --type OWASP \
