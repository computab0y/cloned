...
while getopts u:v:o:s:a:b:e:k:m:i:c:l:n:t:w:d:r: option
do
  case "${option}"
  in
    u) USER_ACC=${OPTARG};;
...
sed -ri "s/<ocpRG>/${OCP_CLUSTER_RG_NAME}/g" install-config.yaml
...
#Replace VNET RG instances
sed -ri "s/<networkRG>/${VNET_RG}/g" install-config.yaml
sed -ri "s/<vnetworkname>/${VNET_NAME}/g" install-config.yaml
...
install_cmd="nohup openshift-install create cluster --dir=${OCP_CLUS_DIR} --log-level=DEBUG > ${OCP_DIR}install-cluster.sh"
