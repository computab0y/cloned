...
SSH_KEY_FILE=./../../ssh-keys/${OCP_INSTANCE}-priv-key.ssh
if [ -f $SSH_KEY_FILE ]
then
    az keyvault secret set --vault-name $shrd_kv_name --name $OCP_INSTANCE-priv-ssh-key --file $SSH_KEY_FILE --encoding ascii
else 
