...
# ******** CREATE ARGOCD APP for storage deployment
resource "kubectl_manifest" "CREATE-ARGOCD-APP-for-storage-deployment" {
    yaml_body = <<YAML
apiVersion: argoproj.io/v1alpha1
kind: Application
metadata:
  name: "${var.storage-app-name}"
...
spec:
  ...
  source:
    ...
    targetRevision: main
    path: "${var.storage-install-source-path}"
  destination:
